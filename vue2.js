...
(function (global, factory) {
  ...
  * */
  ...
  /*判断传入的参数是否是promise，这个判断并不严格，因为只要一个对象同时有then方法和catch方法，都能返回true，
  *比如isPromise({ then () { }, catch () { } }) ，
  返回true，但isPromise传入的参数其实并不是promise */
  ...
  * */
  ...
  function defineReactive$$1 (
    ...
    * */
    ...
    /**getOwnPropertyDescriptor用来返回对象某个属性的描述符 
     * *MDN:https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Reflect/getOwnPropertyDescriptor
     * 如下例子  const obj = {
        name: '古天乐',
        hobby: '是兄弟，就来砍我',
        action: '游戏我只玩传奇'
      }
      const name = Reflect.getOwnPropertyDescriptor(obj, 'name');
      console.log(name)
      // {value: "古天乐", writable: true, enumerable: true, configurable: true}
    */
    var property = Object.getOwnPropertyDescriptor(obj, key);

    /* 判斷該属性是否可修改
    * 针对下面这行代码，当我们处理静态列表时候，可对列表进行Object.freeze(list).这样就不会走下面的
    的方法，起到优化项目的作用
    */
    ...
    那么这个时候{ objValueKey1:{ objValueKey2: objValueValue2 } }对象也会被observe到，
    在observe该对象的时候，{ objValueKey2: objValueValue2 }也会被observe到。
